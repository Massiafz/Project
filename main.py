...
window.columnconfigure(0, weight=0)  # Left side panel column
window.columnconfigure(1, weight=1)  # Main content area
...
login_button.config(command=lambda: open_login_window(window, current_account, buttons))
sign_up_button.config(command=lambda: open_sign_up_window(window, current_account, buttons))
logout_button.config(command=lambda: logout(current_account, buttons))
